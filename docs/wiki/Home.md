# HaberNexus Wiki

**HaberNexus**, yapay zeka destekli, tam otomatik bir haber agregasyon platformudur. Bu wiki, projenin kurulumu, yapılandırması ve kullanımı hakkında kapsamlı bilgiler içerir.

## Hızlı Başlangıç

Tek komutla kurulum için:

```bash
wget -qO- https://raw.githubusercontent.com/sata2500/habernexus-nextjs/master/scripts/install.sh | bash
```

## İçindekiler

### Kurulum ve Yapılandırma

- [Deployment Rehberi](DEPLOYMENT_GUIDE.md) - Ubuntu sunucuya kurulum
- [Environment Değişkenleri](ENVIRONMENT_VARIABLES.md) - Yapılandırma seçenekleri
- [Google OAuth Kurulumu](GOOGLE_OAUTH_SETUP.md) - OAuth yapılandırması

### Kullanım

- [Admin Panel Kullanımı](ADMIN_PANEL.md) - Yönetim paneli rehberi
- [RSS Kaynakları](RSS_FEEDS.md) - Haber kaynakları ekleme
- [AI İçerik Motoru](AI_CONTENT_ENGINE.md) - Otomatik içerik üretimi

### Geliştirme

- [Katkıda Bulunma](https://github.com/sata2500/habernexus-nextjs/blob/main/CONTRIBUTING.md) - Geliştirici rehberi
- [AI Development Guide](https://github.com/sata2500/habernexus-nextjs/blob/main/docs/ai-agents/AI_DEVELOPMENT_GUIDE.md) - AI ajanları için rehber
- [Tech Stack](TECH_STACK.md) - Kullanılan teknolojiler

### Sorun Giderme

- [Sık Sorulan Sorular](FAQ.md)
- [Bilinen Sorunlar](KNOWN_ISSUES.md)

## Destek

Sorularınız veya sorunlarınız için:

- [GitHub Issues](https://github.com/sata2500/habernexus-nextjs/issues)
- E-posta: salihtanriseven25@gmail.com

## Lisans

Bu proje MIT lisansı altında lisanslanmıştır.
